...
// 각 사진에 찍힌 인물의 이름을 담은 이차원 문자열 배열 photo가 매개변수로 주어질 때, 사진들의 
...
export default function solution(name, yearning, photo) {
    const answer = [];
    const obj = {}
    ...
    for (let i = 0; i < photo.length; i++) {
        answer.push(photo[i].reduce((a, b) => {
            console.log(a, b)
            return a + (obj[b] || 0)
        }, 0))
    }
...
console.log(solution(["may", "kein", "kain", "radi"], [5, 10, 1, 3], [["may", "kein", "kain", "radi"], ["may", "kein", "brin", "deny"], ["kon", "kain", "may", "coni"]])) // [16]